...
const NewChat = (props) => {
    ...
    const searchuser = val => {
        setSearch(val);
        const regex = new RegExp(val, 'i'); // 'i' flag for case-insensitive matching
        const filteredUsers = allUserBackup.filter(it => {
            // console.log(it);
            // if (typeof it.name !== 'string') {
            //     console.error("Name is not a string:", it.name);
            //     return false; // Exclude this item from the filter
            // }
            if(it && it.name){

                return it.name.match(regex);
            }
        });
                ...
                <ScrollView contentContainerStyle={{paddingBottom:80}}>
                    ...
                    {allUsers.length == 0 ?
                        <View style={{alignItems:'center'}}>
                            <Text style={{...FONTS.h5,marginTop:20,marginBottom:5,color:colors.title}}>No user found</Text>
                        </View>
                        :
                        null
                    }
